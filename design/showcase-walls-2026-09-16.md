...
## static (01)
- BUG: k1_test cwd race: compiler.rs:1360 CwdGuard::enter(home_dir) chdirs the whole process per module; k1_test compiles cases on parallel threads -> relative file reads in #static/globals race. Repro: scratchpad/static_embed/ under k1_test with another case.
...
- Minor: `c-int` must be `core/c-int` in fn main; no string.to-int (i64/from-string).
...
- (repeat) k1_test filter ignores dirs.
...
- (repeat) #debug on macro; crash in macro -> generic diagnostic at macro def.
...
- `--chatty` prints "58.1M instrs (0.00us/instr)": per-instruction time rounds to zero; want ns/instr or Minstr/s.
