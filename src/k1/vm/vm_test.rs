...
mod stack_frame_tests {
    ...
    #[test]
    fn test_push_padding_bytes() {
        let mut vm = test_vm();
        let frame = vm.current_frame_mut();
        frame.push_padding_bytes(4);
        assert_eq!(frame.current_offset_bytes(), 4);
        assert_eq!(frame.to_bytes(), [0u8; 4]);
    }
    ...
    fn test_push_integer() {
        ...
        let tests = vec![
            (TypedIntegerValue::U8(42), 1),
            (TypedIntegerValue::I8(-42), 1),
            (TypedIntegerValue::U16(1234), 2),
            (TypedIntegerValue::I16(-1234), 2),
            (TypedIntegerValue::U32(12345678), 4),
            (TypedIntegerValue::I32(-12345678), 4),
            (TypedIntegerValue::U64(12345678901234), 8),
            (TypedIntegerValue::I64(-12345678901234), 8),
        ];

        for (value, expected_size) in tests {
            let start_len = frame.current_offset_bytes();
            frame.push_integer(value);
            assert_eq!(frame.current_offset_bytes() - start_len, expected_size);
        }
    }
    ...
    fn test_push_value_basic() {
        ...
        frame.push_value_no_align(Value::Unit);
        assert_eq!(frame.current_offset_bytes(), 1);
        assert_eq!(frame.to_bytes()[0], crate::typer::UNIT_BYTE_VALUE);

        frame.push_value_no_align(Value::Bool(true));
        assert_eq!(frame.to_bytes()[1], 1);

        frame.push_value_no_align(&Value::Char(b'A'));
        assert_eq!(frame.to_bytes()[2], b'A');

        let int_ptr =
            frame.push_value_no_align(&Value::Integer(TypedIntegerValue::U32(42))) as *const i32;
    ...
    fn test_push_value_float() {
        ...
        let f32ptr = frame.push_value_no_align(&f32_val) as *const f32;
        assert_eq!(frame.current_offset_bytes(), 4);
        ...
        let f64_ptr = frame.push_value_no_align(&f64_val) as *const f64;
        ...
        let f64_ptr_aligned = frame.push_value_no_align(&f64_val) as *const f64;
    ...
    fn test_push_raw_copy() {
        ...
        let src_ptr = slice_from_raw_parts(source.as_ptr(), source.len());

        let result_ptr = frame.push_raw_copy(4, 4, src_ptr);
        assert_eq!(frame.current_offset_bytes(), 4);
