...
impl TypedModule {
        ...
        for type_param in parsed_type_defn.type_params.iter() {
            ...
            if !added {
                return failf!(
                    ...
                    self.name_of(type_param.ident).blue()
                );
            }
        }
    ...
    fn eval_expr_inner(
        ...
        match expr {
            ParsedExpression::ListLiteral(list_expr) => {
                ...
                let list_new_fn_call = self.synth_function_call(
    ...
    fn eval_for_expr(
        ...
        let iteration_element_val_def = TypedStmt::Let(Box::new(LetStmt {
            ...
            initializer: if is_string_iteree {
                ...
                self.synth_function_call(
                    NamespacedIdentifier {
                        namespaces: vec![get_ident!(self, "List")],
                        name: get_ident!(self, "get"),
                        span: body_span,
                    },
                    body_span,
        ...
        match fn_call.is_method {
            ...
            false => {
                ...
                if let Some(function_id) = self.scopes.find_function_namespaced(
                    ...
                    let fn_not_found =
                        || failf!(fn_call.span, "Function not found: '{}'", self.name_of(fn_name));
    ...
    fn solve_generic_params(
        ...
        match (self.types.get(passed_expr), self.types.get(argument_type)) {
            (_, Type::TypeVariable(tv)) => {
                ...
                // specializing List.grow on List.push.T = List.filter.U
                        ...
                        self.check_types(existing_solution.type_id, solved_param.type_id, scope_id)
                    {
                        return make_fail_span(
                            format!(
                                ...
                                self.name_of(solved_param.name),
                                msg
                            ),
                            span,
                        );
                        ...
                        debug!(
                            ...
                            self.name_of(existing_solution.name)
                        )
                    }
                } else {
                    debug!(
    ...
    fn eval_function_predecl(
        ...
        trace!(
            ...
            self.name_of(parsed_function.name),
            ...
            if idx == 0 && !specialize {
                ...
                if is_ability_decl {
                    let type_is_self = self
                        ...
                        .get(type_id)
                        .as_tvar()
                        .map(|tvar| self.name_of(tvar.name) == "Self")
                        .unwrap_or(false);
