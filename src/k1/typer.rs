...
#[derive(Debug, Clone)]
pub struct TypedSumIsVariantExpr {
    pub sum_expr: TypedExprId,
    pub variant_index: u32,
    pub span: SpanId,
}
impl_copy_if_small!(16, TypedSumIsVariantExpr);
...
pub struct TypedProgram {
    ...
    /// at this exact layout level. If we ever find ourselves in the current stack entry,
    /// that means we have an infinite layout: a mention of a type directly within itself
    /// for example: deftype Foo = { f: Foo }
    /// If we ever find ourselves at a prior level, that's a valid recursive mention
    /// for example: deftype Foo = { f: *Foo }
    ///                                 ^ the * pushes a new entry on the stack
    /// What about a flat Vec with "layout_depth_level: usize" instead?
...
impl TypedProgram {
    ...
    pub fn get_stmt_type(&self, stmt: TypedStmtId) -> TypeId {
        match self.stmts.get(stmt) {
            ...
            | TypedStmt::Require(_)
            | TypedStmt::Defer(_) => self.types.builtins.empty,
        }
    }
    ...
    fn eval_type_defn(
        ...
        let Some(defn_stack_entry) = self.type_defn_stack.pop() else {
            self.ice_with_span("No defn stack entry", parsed_type_defn.span);
        };
    ...
    fn get_module_manifest(
        ...
        match manifest_value {
            StaticValue::Struct(value) => {
                ...
                let kind = self.static_values.get(value_fields[0]).as_sum().unwrap();
    ...
    fn eval_pattern(
        ...
        match parsed_pattern_expr {
            ...
            ParsedPattern::Sum(sum_pattern) => {
                ...
                let Some(sum_type) = self.types.get(target_type_id).as_sum() else {
                    return failf!(
                        sum_pattern.span,
                        "sum pattern will never match {}",
                        self.type_id_to_string(target_type_id)
                    );
                };
                ...
                let Some(matching_variant) =
                    self.types.sum_variant_by_name(sum_type.variants, sum_pattern.variant_name)
                else {
                    return failf!(
                        sum_pattern.span,
    ...
    pub fn check_types(
        ...
        match (self.types.get(expected), self.types.get(actual)) {
            ...
            (Type::Lambda(expected_lambda_id), Type::Lambda(actual_lambda_id)) => {
                let expected_lambda = self.types.lambda_types.get(*expected_lambda_id);
                let actual_lambda = self.types.lambda_types.get(*actual_lambda_id);
    ...
    fn execute_static_expr_with_vm(
        ...
        if let Some(shortcut_value_id) = self.eval_trivial_static_expr(parsed_expr, ctx)? {
            ...
            //    self.static_value_to_string(shortcut_value_id),
    ...
    pub fn eval_global_body(&mut self, parsed_global_id: ParsedGlobalId) -> K1Result<()> {
        ...
        let expected_type_for_execution = match self.types.get_static_type_of_type(declared_type) {
    ...
    fn eval_field_access(
        ...
        if let Some(sum_result) = self.handle_sum_constructor(
        ...
        // Optional fork case: .tag sum special accessor
    ...
    fn eval_expr_inner(
        ...
        match expr {
            ...
            ParsedExpr::AnonSumConstructor(anon_sum) => {
                ...
                let Some(result) = self.handle_sum_constructor(
                    None,
                    ...
                    self.ice_with_span("handle_sum_constructor should never return Ok(None) when called in anonymous mode", span)
                };
                Ok(result)
            }
        ...
        let elements: MList<TypedExprId, MemTmp> = {
            ...
            for elem in parsed_elements.iter() {
                ...
                if element_type.is_none() {
                    ...
                    let chased_type = self.types.get_static_family_id_if_static(this_element_type);
        ...
        let expected_type_for_execution = match kind {
            ParsedStaticBlockKind::Value => match ctx.expected_type_id {
                ...
                Some(expected_type_id) => {
                    match self.types.get_static_type_of_type(expected_type_id) {
                ...
                // compilation fast
                {
                    ...
                    if expected_arm_type_id.is_none() && consequent_expr_type != NEVER_TYPE_ID {
                        ...
                        let chased_consequent_id =
                            self.types.get_static_family_id_if_static(consequent_expr_type);
    ...
    fn eval_for_expr(&mut self, for_expr: &ForExpr, ctx: EvalExprContext) -> K1Result<TypedExprId> {
        ...
        let next_is_some_call = self.synth_sum_is_variant(
            next_variable.variable_expr,
            1,
            false,
    ...
    fn eval_if_expr(
        ...
        let consequent = if if_expr.alt.is_none()
            && !cons_original_never
            && consequent_original_type != self.types.builtins.empty
        {
        ...
        let alternate = if let Some(parsed_alt) = if_expr.alt {
            let type_hint = if cons_never {
                ...
                let consequent_type_chased =
                    self.types.get_static_family_id_if_static(consequent_type);
    ...
    fn resolve_parsed_function_call_method(
        ...
        if let Some(base_arg) = first_arg {
            if let Some(sum_constr) = self.handle_sum_constructor(
                Some(base_arg.value),
                fn_name,
        ...
        if let Some(sum_as_result) =
            self.handle_sum_as_variant_call(base_expr, call, ctx.with_no_expected_type())?
        {
    ...
    fn handle_sum_as_variant_call(
        ...
        let condition =
            self.synth_sum_is_variant(base_expr, variant_index, is_reference, ctx, Some(span))?;
    ...
    fn handle_sum_constructor(
        ...
        let base_type_id = match base_expr {
            Some(base_expr) => {
                ...
                match self.types.get(base_type_in_scope) {
                    Type::Sum(_e) => base_type_in_scope,
                    Type::Generic(g) => {
            ...
            None => {
                ...
                let base_sum_type_id = match self.types.get_instance_info(sum_type_id) {
                    None => sum_type_id,
        ...
        if let Type::Sum(e) = base_sum_or_generic_sum {
        ...
        } else if let Type::Generic(g) = base_sum_or_generic_sum {
    ...
    fn extract_function_type_from_functionlike(&self, typ: &Type) -> Option<TypeId> {
        ...
        match typ {
            ...
            Type::Lambda(lam_id) => Some(self.types.lambda_types.get(*lam_id).function_type),
    ...
    fn assert_builtin_types_correct(&self) {
        ...
        // Eventually we need a better way of doing this
        {
            let ordering_enum = self.types.get(ORDERING_TYPE_ID).expect_sum();
    ...
    pub fn get_struct_layout(&mut self, struct_type_id: TypeId) -> SV4<StructField> {
        ...
        if struct_pt.is_empty() {
            ...
            self.types.get_agg_struct_layout(struct_pt.expect_agg())
        }
    }
    ...
    fn get_type_schema(&mut self, type_id: TypeId, span: SpanId) -> StaticValueId {
        ...
        let int_kind_enum = self.types.get(int_kind_type_id).expect_sum();
        ...
        let chased_type_id = self.types.get_static_family_id_if_static(type_id);

        let typ = self.types.get(chased_type_id);
        let static_sum = match typ {
            ...
            Type::Integer(integer_type) => {
                ...
                let payload_value_id =
                    self.static_values.add(StaticValue::Sum(int_kind_enum_value));
                let enum_value =
            ...
            Type::Sum(typed_sum) => {
                ...
                let tag_type = self.types.get(typed_sum.tag_type).expect_integer();
                let tag_type_value_id =
                    self.static_values.add(StaticValue::Sum(TypedProgram::make_int_kind(
                        self.types.mem.getn(int_kind_enum.variants),
                        int_kind_type_id,
                        tag_type,
                    )));
                let sum_agg_id = self.get_physical_type(type_id).unwrap().expect_agg();
                let sum_pt = self.types.agg_types.get(sum_agg_id).agg_type.expect_sum();
                let maybe_payload_offset = sum_pt.payload_offset;
        ...
        let static_value_id = self.static_values.add(StaticValue::Sum(static_sum));
