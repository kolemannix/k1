...
use crate::typer::{
    ArithOpClass, ArithOpOp, AssignmentKind, BitwiseBinopKind, Builtin, Call, CallId, Callee,
    CastType, FieldAccessKind, FunctionId, IntegerCastDirection, Layout, LetStmt,
    Linkage as TyperLinkage, LoopExpr, MatchingCondition, MatchingConditionInstr,
    StaticContainerKind, StaticValue, StaticValueId, TypedBlock, TypedCast, TypedExpr, TypedExprId,
    TypedFloatValue, TypedFunction, TypedGlobalId, TypedIntValue, TypedMatchExpr, TypedProgram,
    TypedStmt, TypedStmtId, VariableId, WhileLoop,
};
use crate::{SV8, bc, kmem};
...
impl<'ctx, 'module> Codegen<'ctx, 'module> {
    ...
    fn codegen_static_value_canonical(
        ...
        let result = match v {
            ...
            StaticValue::Zero(type_id) => {
                ...
                match self.k1.types.is_aggregate(*type_id) {
                    true => {
                        todo!(
    ...
    fn codegen_function_call(&mut self, call_id: CallId) -> CodegenResult<LlvmValue<'ctx>> {
        ...
        if let Some(intrinsic_type) = typed_function.and_then(|f| f.intrinsic_type) {
            if !matches!(bc::intrinsic_handler(intrinsic_type), BuiltinHandler::Backend(_)) {
        ...
        for (index, arg_expr) in call.args.iter().enumerate() {
            ...
            let param_k1_ty =
                *self.mem.get_nth_lt(llvm_function_type.param_k1_types, logical_index);
            let abi_mapping =
                *self.mem.get_nth_lt(llvm_function_type.param_abi_mappings, logical_index);
        ...
        match intrinsic_type {
            ...
            Builtin::BitwiseBinop(op_kind) => {
                ...
                let result = match op_kind {
                    BitwiseBinopKind::And => self.builder.build_and(lhs, rhs, ""),
                    BitwiseBinopKind::Xor => self.builder.build_xor(lhs, rhs, ""),
                    BitwiseBinopKind::Or => self.builder.build_or(lhs, rhs, ""),
            ...
            Builtin::CompilerMessage => Ok(self.builtin_types.unit_basic().into()),
    ...
    fn codegen_intrinsic_function_body(
        ...
        let instr = match intrinsic_type {
            ...
            Builtin::TypeName => {
                ...
                let return_ty = llvm_fn.function_type.return_k1_type;
                let entry_block = self.builder.get_insert_block().unwrap();
                    ...
                    .sorted_unstable_by_key(|(type_id, _)| type_id.as_u32())
                {
                    ...
                    let value_marshalled =
                        self.marshal_abi_param_value(return_abi, &return_ty, value, true);
